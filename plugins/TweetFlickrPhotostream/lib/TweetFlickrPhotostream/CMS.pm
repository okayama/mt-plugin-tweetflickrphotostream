package TweetFlickrPhotostream::CMS;
use strict;
...
sub _mode_tweetflickrphotostream_tweet_photo {
    my $app = shift;
    ...
    my $redirect_url;
    my $log = TweetFlickrPhotostream::Util::tweet_flickr_photostream_by_id( $photo_id, $blog_id );
...
sub _mode_tweetflickrphotostream_oauth {
    my $app = shift;
    my $plugin = MT->component( 'TweetFlickrPhotostream' );
...
sub _mode_tweetflickrphotostream_get_access_token {
    my $app = shift;
    my $plugin = MT->component( 'TweetFlickrPhotostream' );
    my $blog = $app->blog or return $app->errtrans( 'Invalid request.' );
    my $blog_id = $blog->id;
    ...
    my $http_header = HTTP::Headers->new( 'Authorization' => $request->to_authorization_header );
    my $http_request = HTTP::Request->new( $request_method, $request_url, $http_header );
    $http_request->content( $request->to_post_body . '&oauth_verifier=' . $app->param( 'tweetflickrphotostream_pin' ) );
